...
def wasd(kitt):
    ...
    while True:
        ...
        if event.event_type == keyboard.KEY_DOWN and event.name == 'z':
            ...
            y = kitt.record(int(44100 * duration))      # record for 2 seconds(2 * Fs samples)
            real_x = 436
            real_y = 314
            #wavfile.write("verse_recordings/10-jun-x436-y314.wav", 44100, y)
            #loc.prepare_ref("ref tests/.wav")
            estimated_x, estimated_y = loc.locate(y, real_x, real_y)
